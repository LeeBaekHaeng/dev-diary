...
- [[포털 사이트][설문응답결과] 롬복 생성자 기반 종속성 주입](#포털-사이트설문응답결과-롬복-생성자-기반-종속성-주입)
...
### [포털 사이트][설문문항] 롬복 생성자 기반 종속성 주입

[2024년 전자정부 표준프레임워크 컨트리뷰션][포털 사이트][설문문항] 롬복 생성자 기반 종속성 주입

- Source > Format
- `@Repository("DAO")` 를 `@Repository` 로 수정
- `@Service("Service")` 를 `@Service` 로 수정
- `@RequiredArgsConstructor` 추가
- ` *   2024.09.25  이백행          컨트리뷰션 롬복 생성자 기반 종속성 주입` 개정이력 수정

크롬 링크 주소 복사
```
http://localhost:8080/pst_webapp/uss/olp/qqm/EgovQustnrQestnManageList.do
```

검색
```
/uss/olp/qqm/EgovQustnrQestnManageList.do
```

브랜치 생성
```
2024/di/EgovQustnrQestnManageController
```
...
//	private QustnrQestnManageDao dao;
	private final QustnrQestnManageDao dao;

//	@Resource(name = "egovQustnrQestnManageIdGnrService")
//	private EgovIdGnrService idgenService;
	private final EgovIdGnrService egovQustnrQestnManageIdGnrService;
```
...
//	private EgovQustnrQestnManageService egovQustnrQestnManageService;
	private final EgovQustnrQestnManageService egovQustnrQestnManageService;
...
https://github.com/LeeBaekHaeng/egovframe-portal-site-template/commits/2024/di/EgovQustnrQestnManageController/
...
### [포털 사이트][설문응답결과] 롬복 생성자 기반 종속성 주입

[2024년 전자정부 표준프레임워크 컨트리뷰션][포털 사이트][설문응답결과] 롬복 생성자 기반 종속성 주입
...
크롬 링크 주소 복사
```
http://localhost:8080/pst_webapp/uss/olp/qrm/EgovQustnrRespondManageList.do
```

검색
```
/uss/olp/qrm/EgovQustnrRespondManageList.do
```

브랜치 생성
```
2024/di/EgovQustnrRespondManageController
```
